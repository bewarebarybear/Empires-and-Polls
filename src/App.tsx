...
function SummonSimulator() {
  ...
  const sampleNames = {
    Legendary: ['Hathor', 'Gestalt', 'Aramis', 'Ludwig', 'Khufu', 'Jove'],
    Epic: ['Rigard', 'Melendor', 'Kiril', 'Boldtusk', 'Grimm', 'Sabina', 'Sonya'],
    Rare: ['Bane', 'Hawkmoon', 'Berden', 'Valen', 'Dawa', 'Prisca', 'Azar'],
  };
  ...
  const handleSummon = () => {
    ...
    for (let i = 0; i < pullType; i++) {
      ...
      const nameList = sampleNames[rarity];
      const randomName = nameList[Math.floor(Math.random() * nameList.length)];
      generatedPulls.push({ rarity, color, name: `${rarity}: ${randomName}` });
    }
      ...
      {results.length > 0 && (
        <div className="mt-6 pt-6 border-t border-zinc-800">
          <div className="text-xs font-semibold text-amber-400 mb-3 uppercase tracking-wider">
            Summon Results ({pullType === 1 ? '1' : `${Math.min(currentIndex, results.length)} / ${results.length}`})
          </div>

          <div className={`grid gap-2 max-h-64 overflow-y-auto pr-1 ${pullType === 1 ? 'grid-cols-1' : 'grid-cols-2 sm:grid-cols-3'}`}>
            {results.slice(0, pullType === 1 ? 1 : currentIndex).map((res, idx) => (
              <div
                key={idx}
                className={`p-3 rounded-xl border text-center font-bold text-xs sm:text-sm ${res.color}`}
              >
                {res.name}
              </div>
            ))}
